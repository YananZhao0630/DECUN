function [x,cost] = deconv_hqs_conver(y, k, convergence_base_gradient_filter,convergence_base_beta,options)
...
%     +normrnd(Mu, Sigma, 2, 2);
    ...
    iter = 0;
    while iter < max_iter
        betal = beta + convergence_base_beta^(iter+1);
        if is_isotropic
       ...
       % gx = conv2(x, fx+(convergence_base_gradient_filter^(iter+1)).*fspecial('gaussian',[2 2], sigma));
        fxl = fx +(convergence_base_gradient_filter^(iter+1)).*ones(2);
        gx = conv2(x, fxl);
...
%         gx = conv2(x, fx+(1/2^((iter+1))));
       % gy = conv2(x, fy+(convergence_base_gradient_filter^(iter+1)).*fspecial('gaussian',[2 2], sigma));
        fyl = fy+(convergence_base_gradient_filter^(iter+1)).*ones(2);
        gy = conv2(x, fyl);
